...
print(f"Hello User {tf.__version__}")
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
import cv2

tuberculosis_bp = Blueprint('tuberculosis', __name__)

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
UPLOAD_FOLDER = os.path.join(BASE_DIR, '../static/uploads')
PROCESSED_FOLDER = os.path.join(BASE_DIR, '../static/processed')
MODEL_PATH = os.path.join(BASE_DIR, '../models/tb_detection_model.h5')
...
@tuberculosis_bp.route('/tuberculosis')
def home():
    return render_template('tuberculosis.html')

@tuberculosis_bp.route('/tuberculosis/predict', methods=['POST'])
def predict():
